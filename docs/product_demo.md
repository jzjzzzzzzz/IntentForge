...
The CLI can produce a scoped assurance case after the existing deterministic workflow and package it for downstream review. This adds traceability; it does not expand the registered manifest scope or claim engineering certification.
...
Phase 27 adds local content-addressed storage and optional chronological lineage on the same CLI/library boundary. It is not a blockchain service, signature authority, or database, and it does not change CAD generation.
